...
bool Similiar(BiTree A, BiTree B) {
    if (!A && !B) return true;
    if (!A || !B) return false;
    return Similiar(A->lchild, B->lchild) && Similiar(A->rchild, B->rchild);
}
...
int main() {
    ...
    // 构建二叉树
    BiTree A = BuildTree(preorder, inorder, 0, 0, strlen(inorder) - 1);
   ...
   // 输出后序遍历结果
    ...
    // 构建二叉树
    BiTree B = BuildTree(preorder, inorder, 0, 0, strlen(inorder) - 1);
